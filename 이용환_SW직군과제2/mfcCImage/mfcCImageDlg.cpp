...
BEGIN_MESSAGE_MAP(CmfcCImageDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_ACTION1, &CmfcCImageDlg::OnBnClickedBtnAction1)
...
CString g_strFileImage = (_T("E:\\code\\MFC\\image\\save.bmp"));
...
void CmfcCImageDlg::OnBnClickedBtnAction1()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	for (int i = 0; i < 640; i++) {
		moveRect();
...
void CmfcCImageDlg::moveCir()
{
	...
	drawCircle(fm, nSttX + 1, nSttY + 1, nRadius, nGray); 
	...
	for (int j = y; j < y + nRadius * 2; j++) {
		for (int i = x; i < x + nRadius * 2; i++) {
			if (isInCircle(i, j, nCenterX, nCenterY, nRadius)) {
				// 좌표가 유효한 범위 내에 있는지 체크 (validImgPos 함수를 사용하거나 원하는 방식으로 검사)
				if (validImgPos(i, j)) { // 이 조건을 추가하여 유효한 이미지 좌표만 처리하도록 함
					fm[j * nPitch + i] = nGray;
				}
			}
...
void CmfcCImageDlg::OnBnClickedBtnRandom()
{
	...
	// 버튼 클릭 시 호출되는 함수
		...
		// 랜덤한 시작 위치 생성 (0부터 이미지 크기까지의 범위에서 랜덤 값)
		int startX = rand() % nWidth;
		int startY = rand() % nHeight;

		int rectWidth = 32;  // 사각형 너비
		int rectHeight = 24; // 사각형 높이
